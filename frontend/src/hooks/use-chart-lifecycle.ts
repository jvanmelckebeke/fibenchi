...
/**
 * Observe `container` and push its width to `chart` on resize (optionally
 * refitting the time scale); returns a cleanup that disconnects the observer and
 * removes the chart. The option-agnostic primitive used by the bespoke
 * sparkline / intraday charts, which opt out of the full themed lifecycle below.
 */
export function attachResizeAndCleanup(
  ...
  const resizeObserver = new ResizeObserver((entries) => {
    for (const entry of entries) {
      ...
      if (opts.refit) chart.timeScale().fitContent()
    }
  })
