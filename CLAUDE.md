...
- `services/market_calendar.py` — `MarketCalendarService` (singleton `market_calendar`): maps Yahoo symbols to `exchange_calendars` venue calendars (suffix + index tables; unmapped → `None`); symbol shape is only ever interpreted in `calendar_name()`. Supplies exact session dates to the σ-Move gap guard (`compute_indicators(df, session_dates=...)`) so holidays aren't mistaken for feed holes. Schedule questions go through the `Venue` facade (`market_calendar.venue(symbol)`, cached per calendar): `is_open`/`next_open`/`next_close`/`previous_close` and `phase()` (`premarket|open|aftermarket|closed`; extended windows are `EXTENDED_HOURS` data anchored to actual open/close, half-day-aware, US-only). Fail-safe: unknown venue or out-of-range query returns `None` and callers fall back to business-day heuristics; the quote feed's `market_state` remains the live authority.
