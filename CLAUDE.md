...
- `services/compute/` — Computational logic, separated from I/O:
  ...
  - `fx_close.py` — `normalize_fx_close(df)` rewrites an FX (`=X`) frame's closes as the following bar's open. Yahoo's settled FX daily bar puts the session open in its `close` field, so candles are bodyless and every close-based indicator runs a session stale. Stateless like `splits.py`: it re-decides from the frame's own median candle body each fetch, so it stops firing on its own if Yahoo starts publishing a real close.
