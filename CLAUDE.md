...
- **The contract is a single source of truth:** `backend/app/schemas/companion.py` (`CompanionConfig`, `CompanionCalendar`) is the one definition. `backend/scripts/export_companion_schema.py` emits `backend/companion.schema.json` and `backend/companion.calendar.schema.json`, which feed the app's Zod codegen — so the contract can't drift between Python and TypeScript. Each model carries its own `version: Literal[1]` field the app gates on, bumped independently on any breaking shape change to that bundle (config v1 is intentionally minimal: groups/tickers/tags; pseudo-ETFs/settings would be a later version).
