...
- `services/compute/` — Computational logic, separated from I/O:
  ...
  - `normalize.py` — `normalize_frame(df, symbol)`, the one pipeline every provider frame goes through. Steps are `(df, symbol) -> df` functions: `UNIVERSAL` runs for everything, `BY_KIND` holds the ones a single `AssetKind` needs (registry pattern, like `INDICATOR_REGISTRY`). A new provider quirk is one entry plus a function — never a kind test scattered into the fetch path, and never a step guessing which instruments it may touch.
  - `fx_close.py` — `recover_fx_close(df)` rewrites an FX frame's closes as the following bar's open. Yahoo's settled `=X` bar puts the session open in its `close` field, so candles are bodyless and every close-based indicator runs a session stale. Reached only via `BY_KIND[AssetKind.FX]`; stateless like `splits.py`, re-deciding from the frame's own median candle body each fetch, so it stops firing on its own if Yahoo starts publishing a real close.
