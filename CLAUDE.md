...
- `services/market_calendar.py` — `Symbol` (a `str` subclass, drop-in for plain ticker strings) resolves its own venue: `Symbol("IWDA.AS").venue` → a cached per-calendar `Venue` facade over `exchange_calendars` (suffix + index tables; unmapped → `None`); ticker shape is only ever interpreted in `Symbol.calendar_name`. `Venue` answers session questions for the σ-Move gap guard (`session_dates_for_index` → `compute_indicators(df, session_dates=...)`, so holidays aren't mistaken for feed holes) and schedule questions: `is_open`/`next_open`/`next_close`/`previous_close` and `phase()` (`premarket|open|aftermarket|closed`; extended windows are `EXTENDED_HOURS` data anchored to actual open/close, half-day-aware, US-only). Fail-safe: unknown venue or out-of-range query returns `None` and callers fall back to business-day heuristics; the quote feed's `market_state` remains the live authority.
