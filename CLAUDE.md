...
- **Fibenchi is only the "config plane":** it tells the app *what to track* (groups / tickers / tags) via `GET /api/companion/config`, and serves the venue trading calendar the app can't compute (no maintained `exchange_calendars` equivalent exists in JS) via `GET /api/companion/calendar`. Those two endpoints are the **only** coupling between the two codebases. They are deliberately separate: the calendar changes about once a year, group config whenever a group is touched, so they carry independent version constants (`CONFIG_VERSION` / `CALENDAR_VERSION`) and must not invalidate each other's cache.
- **The contract is a single source of truth:** `backend/app/schemas/companion.py` (`CompanionConfig`, `CompanionCalendar`) is the one definition. `backend/scripts/export_companion_schema.py` emits `backend/companion.schema.json` and `backend/companion.calendar.schema.json`, which feed the app's Zod codegen — so the contract can't drift between Python and TypeScript. The model carries a `version: Literal[1]` field the app gates on; any breaking shape change is a version bump (v1 is intentionally minimal: groups/tickers/tags; pseudo-ETFs/settings would be a later version).
...
- `domain/` — application-level models that are neither DB rows (`models/`) nor API contracts (`schemas/`); anything may depend on this layer. `domain/instrument.py`: `classify(ticker) -> Instrument` (`AssetKind` + calendar + currency, decided in one pass — the single place ticker shape is interpreted; downstream asks `ref.kind.is_future` etc., never re-inspects characters; `classify` lazy-imports the listing tables, the one seam keeping domain ↔ market_calendar package inits acyclic). `AssetRef` (`domain/assetref.py`) is *the* asset identity in application code: a `str` subclass (drop-in for plain ticker strings — equality/hash are the ticker's) that knows its instrument traits (`AssetRef("IWDA.AS").venue`, `.currency`, `.calendar_name`, `.kind` — an `AssetKind`: equity/index/crypto/fx/future/unknown) and optionally carries the stored asset `id` (`AssetRef.of(asset)`, built while the ORM row is live). Pipelines and batch loops carry `AssetRef`, never ORM `Asset` rows or loose `(asset_id, symbol)` scalars — ORM instances expire on rollback/commit mid-batch-loop and then raise MissingGreenlet on attribute access.
- `services/market_calendar/` — venue trading-calendar package under `AssetRef`: `listings.py` (pure venue trait data: `SUFFIX_LISTINGS` with calendar+currency per suffix, index table, `EXTENDED_HOURS`), `venue.py`, `schedule.py` (`any_venue_open` gate, `schedule_poll_hint`); public API re-exported from `__init__`. `Venue` (cached per calendar, over `exchange_calendars`) answers session questions for the σ-Move gap guard (`session_dates_for_index` → `compute_indicators(df, session_dates=...)`, so holidays aren't mistaken for feed holes) and schedule questions: `is_open`/`next_open`/`next_close`/`previous_close` and `phase()` (`premarket|open|aftermarket|closed`; extended windows are `EXTENDED_HOURS` data anchored to actual open/close, half-day-aware, US-only). Fail-safe: unknown venue or out-of-range query returns `None` and callers fall back to business-day heuristics; the quote feed's `market_state` remains the live authority.
...
- **SSE quote stream:** Backend pushes quotes for assets in any group via SSE (`AssetRepository.list_in_any_group_refs`) with adaptive intervals (15s during market hours, 60s pre/post, 300s closed). Frontend reconnects with exponential backoff (1s→30s) on disconnect.
