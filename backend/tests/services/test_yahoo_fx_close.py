...
Which symbols reach this step is ``normalize.BY_KIND``'s job — see
test_normalize.py. These tests are about what it does once it is reached.
"""
...
class TestRecovery:
    def test_close_becomes_the_next_bar_open(self):
        out = recover_fx_close(frame(DEFECTIVE), "JPY=X")
    ...
    def test_rerunning_on_the_same_provider_frame_is_stable(self):
        df = frame(DEFECTIVE)
        assert recover_fx_close(df, "JPY=X").equals(
            recover_fx_close(df.copy(), "JPY=X")
        )
...
class TestGating:
    def test_a_frame_with_real_bodies_is_left_alone(self):
        ...
        assert recover_fx_close(df, "JPY=X") is df
    ...
    def test_body_ceiling_is_the_boundary(self):
        ...
        for factor, repaired in ((FX_BODY_CEILING / 2, True), (FX_BODY_CEILING * 2, False)):
            ...
            out = recover_fx_close(df, "EURUSD=X")
