...
class TestSafeFloat:
    ...
    async def test_inf_returns_none(self):
        assert _safe_float(float("inf")) is None

    async def test_neg_inf_returns_none(self):
        assert _safe_float(float("-inf")) is None

    async def test_valid_float(self):
        assert _safe_float(15.678, multiplier=1, decimals=2) == 15.68

    async def test_multiplier_applied(self):
        assert _safe_float(0.15, multiplier=100, decimals=1) == 15.0

    async def test_string_number_converted(self):
        assert _safe_float("42.5", multiplier=1, decimals=1) == 42.5

    async def test_invalid_string_returns_none(self):
...
class TestBatchFetchFundamentalsSync:
    ...
    async def test_empty_symbols(self, mock_ticker_cls):
        assert await yahoo_client.fundamentals([]) == {}

    @patch("app.services.yahoo.client.Ticker")
    async def test_returns_all_fields(self, mock_ticker_cls):
        ticker = MagicMock()
        ticker.key_stats = {
    ...
    @patch("app.services.yahoo.client.Ticker")
    async def test_string_error_for_symbol(self, mock_ticker_cls):
    ...
    @patch("app.services.yahoo.client.Ticker")
    async def test_nan_values_filtered(self, mock_ticker_cls):
        ticker = MagicMock()
    ...
    @patch("app.services.yahoo.client.Ticker")
    async def test_multiple_symbols(self, mock_ticker_cls):
        ticker = MagicMock()
        ticker.key_stats = {
            "AAPL": {"forwardPE": 28.5, "pegRatio": 2.1, "enterpriseToEbitda": 22.3},
        ...
        result = await yahoo_client.fundamentals(["AAPL", "MSFT"])
        assert len(result) == 2
